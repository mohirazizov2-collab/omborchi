...
const printReceipt = (sale: any) => {
  ...
  const rows = sale.items?.map((it: any) =>
    `<tr>
      <td style="padding:3px 0;max-width:140px;overflow:hidden">${it.name}</td>
      <td style="text-align:center;padding:3px 6px">${it.quantity}</td>
      <td style="text-align:right;padding:3px 0;white-space:nowrap">${new Intl.NumberFormat("uz-UZ").format(it.price)} so'm</td>
      <td style="text-align:right;padding:3px 0 3px 6px;white-space:nowrap;font-weight:bold">${new Intl.NumberFormat("uz-UZ").format(it.subtotal)} so'm</td>
    </tr>`
  ).join("");
  const date = new Date().toLocaleString("uz-UZ");
  win.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Chek</title>
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    body{font-family:'Courier New',monospace;font-size:12px;color:#000;background:#fff;padding:16px;width:300px}
    .center{text-align:center}.bold{font-weight:bold}.big{font-size:17px}
    .line{border-top:1px dashed #000;margin:8px 0}
    table{width:100%;border-collapse:collapse}
    th{font-size:10px;border-bottom:1px solid #000;padding-bottom:4px;text-align:left}
    th:nth-child(2){text-align:center}th:nth-child(3),th:nth-child(4){text-align:right}
    .total-row td{border-top:2px solid #000;padding-top:6px;font-weight:bold;font-size:14px}
    .footer{margin-top:14px;text-align:center;font-size:10px;color:#555}
    @media print{body{width:100%;padding:8px}}
  </style></head><body>
  <div class="center bold big" style="margin-bottom:2px">OMBORCHI.UZ</div>
  <div class="center" style="font-size:10px;color:#666;margin-bottom:2px">${date}</div>
  ${sale.warehouseName?`<div class="center" style="font-size:10px">🏪 ${sale.warehouseName}</div>`:""}
  <div class="center" style="font-size:10px">Kassir: ${sale.cashierName}</div>
  <div class="center" style="font-size:9px;color:#888">Chek #${sale.id?.slice(-12)}</div>
  <div class="line"></div>
  <table>
    ...
    <tbody>${rows}</tbody>
    <tfoot><tr class="total-row"><td colspan="3">JAMI:</td><td style="text-align:right">${new Intl.NumberFormat("uz-UZ").format(sale.total)} so'm</td></tr></tfoot>
  </table>
  <div class="line"></div>
  <div style="display:flex;justify-content:space-between;margin-bottom:3px"><span>To'lov usuli:</span><span class="bold">${PAY_LABELS[sale.paymentMethod]||sale.paymentMethod}</span></div>
  ${sale.paymentMethod==="cash"?`
  <div style="display:flex;justify-content:space-between;margin-bottom:3px"><span>Berildi:</span><span>${new Intl.NumberFormat("uz-UZ").format(sale.cashGiven||0)} so'm</span></div>
  <div style="display:flex;justify-content:space-between;font-weight:bold;font-size:13px"><span>Qaytim:</span><span>${new Intl.NumberFormat("uz-UZ").format(Math.max(0,sale.change||0))} so'm</span></div>`:""}
  <div class="footer"><div class="line"></div><div>Xarid uchun rahmat! 🙏</div><div style="margin-top:3px">omborchi.uz</div></div>
  ...
  return (
    <>
      <style>{`
        ...
        :root {
          --bg:#0d0f17;--bg2:#141620;--surface:#1a1d2e;--surface2:#212438;--surface3:#272a3f;
          --border:rgba(255,255,255,.06);--border2:rgba(255,255,255,.12);
          --text:#f0f2ff;--text2:#a0a8cc;--muted:#5c6494;
          --accent:#6366f1;--accent2:#818cf8;--accentbg:rgba(99,102,241,.12);
          --success:#10b981;--danger:#ef4444;--warn:#f59e0b;
          --r:14px;--rs:10px;--font:'Outfit',sans-serif;--mono:'JetBrains Mono',monospace;
          --cart-w:340px;
        }
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .root{height:100vh;display:flex;flex-direction:column;background:var(--bg);overflow:hidden}
 
        /* TOPBAR */
        .topbar{height:58px;display:flex;align-items:center;padding:0 20px;gap:12px;background:var(--surface);border-bottom:1px solid var(--border);flex-shrink:0;z-index:100}
        .logo{display:flex;align-items:center;gap:10px;flex-shrink:0;margin-right:6px}
        .logo-mark{width:36px;height:36px;border-radius:10px;background:linear-gradient(135deg,#6366f1,#8b5cf6);display:flex;align-items:center;justify-content:center;font-weight:900;font-size:16px;color:#fff;box-shadow:0 4px 16px rgba(99,102,241,.4)}
        ...
        .tab-btn{padding:6px 18px;border-radius:8px;border:none;background:transparent;color:var(--muted);font-family:var(--font);font-size:13px;font-weight:600;cursor:pointer;transition:all .2s;white-space:nowrap;display:flex;align-items:center;gap:5px}
        .tab-btn:hover{color:var(--text2)}
        .tab-btn.active{background:var(--surface2);color:var(--text);box-shadow:0 1px 6px rgba(0,0,0,.3)}
        .topbar-right{margin-left:auto;display:flex;align-items:center;gap:10px}
        .wh-pill{display:flex;align-items:center;gap:7px;padding:7px 14px 7px 10px;background:var(--accentbg);border:1px solid rgba(99,102,241,.25);border-radius:20px;cursor:pointer;transition:all .2s;flex-shrink:0}
        ...
        .user-chip{font-size:12px;font-weight:600;color:var(--muted);background:var(--surface2);border:1px solid var(--border);padding:5px 12px;border-radius:20px}
 
        /* TOAST */
        .toast{position:fixed;top:68px;left:50%;transform:translateX(-50%);z-index:999;padding:10px 24px;border-radius:30px;font-size:13px;font-weight:700;white-space:nowrap;animation:toastIn .25s cubic-bezier(.34,1.56,.64,1);box-shadow:0 8px 32px rgba(0,0,0,.3)}
        ...
        .wh-bar-count{font-family:var(--mono);font-size:10px;font-weight:600;color:var(--accent2);background:rgba(99,102,241,.15);padding:2px 8px;border-radius:20px}
        .wh-bar-btn{font-size:11px;font-weight:700;color:var(--accent);padding:4px 10px;border-radius:6px;border:1px solid rgba(99,102,241,.25);background:rgba(99,102,241,.08);white-space:nowrap;transition:all .15s}
        ...
        .search-input{width:100%;background:var(--bg2);border:1.5px solid var(--border);border-radius:var(--rs);padding:10px 12px 10px 36px;color:var(--text);font-family:var(--font);font-size:13px;outline:none;transition:all .2s}
        ...
        .cat-pill{padding:5px 13px;border-radius:20px;border:1.5px solid var(--border);background:transparent;color:var(--muted);font-family:var(--font);font-size:11px;font-weight:700;cursor:pointer;transition:all .2s;text-transform:uppercase;letter-spacing:.4px}
        ...
        .ci-del{width:20px;height:20px;background:rgba(239,68,68,.1);border:none;border-radius:5px;color:#f87171;font-size:10px;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0;transition:background .15s}
        .ci-del:hover{background:rgba(239,68,68,.22)}
        .ci-price-btn{width:100%;background:var(--surface3);border:1px solid var(--border);border-radius:7px;padding:6px 9px;display:flex;align-items:center;gap:7px;cursor:pointer;transition:border-color .15s}
        ...
        .qty-btn{width:28px;height:28px;background:var(--surface3);border:1px solid var(--border);border-radius:7px;color:var(--text);font-size:15px;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all .15s;line-height:1;font-family:var(--mono)}
        ...
        .pay-btn{padding:9px 4px;border-radius:9px;border:1.5px solid var(--border);background:var(--surface2);color:var(--muted);font-family:var(--font);font-size:11px;font-weight:700;cursor:pointer;transition:all .2s;display:flex;flex-direction:column;align-items:center;gap:4px}
        ...
        .num-input{width:100%;background:var(--surface2);border:1.5px solid var(--border);border-radius:9px;padding:11px 13px;color:var(--text);font-family:var(--mono);font-size:16px;font-weight:700;outline:none;transition:border-color .2s}
        ...
        .change-row{display:flex;justify-content:space-between;align-items:center;padding:7px 11px;border-radius:8px;font-family:var(--mono);font-size:12px;font-weight:700}
        ...
        .quick-btn{padding:6px 4px;background:var(--surface3);border:1px solid var(--border);border-radius:6px;color:var(--muted);font-family:var(--mono);font-size:10px;font-weight:700;cursor:pointer;transition:all .15s;text-align:center}
        .quick-btn:hover{border-color:rgba(99,102,241,.35);color:var(--accent2);background:var(--accentbg)}
        .checkout-btn{width:100%;padding:14px;border-radius:11px;border:none;background:linear-gradient(135deg,#6366f1,#8b5cf6);color:#fff;font-family:var(--font);font-size:14px;font-weight:800;cursor:pointer;transition:all .2s;letter-spacing:-.2px;box-shadow:0 4px 20px rgba(99,102,241,.3)}
        .checkout-btn:hover:not(:disabled){transform:translateY(-2px);box-shadow:0 8px 30px rgba(99,102,241,.45)}
        ...
        .refresh-btn{padding:8px 16px;background:var(--surface2);border:1px solid var(--border2);border-radius:var(--rs);color:var(--muted);font-family:var(--font);font-size:12px;font-weight:700;cursor:pointer;transition:all .15s}
        ...
        .pay-badge{display:inline-flex;align-items:center;gap:4px;padding:3px 10px;border-radius:20px;font-size:10px;font-weight:700;border:1px solid;font-family:var(--font);margin-top:5px}
        ...
        /* MODALS */
        .overlay{position:fixed;inset:0;background:rgba(0,0,0,.6);display:flex;align-items:center;justify-content:center;z-index:200;padding:16px;backdrop-filter:blur(8px);animation:fadeIn .18s ease}
        @keyframes fadeIn{from{opacity:0}to{opacity:1}}
        .wh-modal{background:var(--surface);border:1px solid var(--border2);border-radius:22px;width:100%;max-width:480px;overflow:hidden;animation:modalUp .25s cubic-bezier(.34,1.56,.64,1);box-shadow:0 24px 64px rgba(0,0,0,.5)}
        @keyframes modalUp{from{transform:translateY(20px) scale(.96);opacity:0}to{transform:translateY(0) scale(1);opacity:1}}
        ...
        .wh-item{display:flex;align-items:center;gap:13px;padding:14px 16px;background:var(--surface2);border:1.5px solid var(--border);border-radius:var(--r);cursor:pointer;transition:all .2s;text-align:left;width:100%;font-family:var(--font)}
        ...
        .wh-item-icon{width:44px;height:44px;border-radius:11px;background:var(--surface3);border:1px solid var(--border2);display:flex;align-items:center;justify-content:center;font-size:20px;flex-shrink:0}
        ...
        .wh-check{width:22px;height:22px;border-radius:50%;border:2px solid var(--border2);display:flex;align-items:center;justify-content:center;font-size:11px;flex-shrink:0;transition:all .2s}
        ...
        .cancel-btn{width:100%;padding:11px;background:var(--surface2);border:1.5px solid var(--border);border-radius:10px;color:var(--muted);font-family:var(--font);font-size:13px;font-weight:700;cursor:pointer;transition:all .15s}
        .cancel-btn:hover{color:var(--text);border-color:var(--border2)}
        .modal-box{background:var(--surface);border:1px solid var(--border2);border-radius:20px;padding:24px;width:100%;max-width:340px;animation:modalUp .25s cubic-bezier(.34,1.56,.64,1);box-shadow:0 24px 64px rgba(0,0,0,.5)}
        .modal-title{font-size:17px;font-weight:800;letter-spacing:-.3px;margin-bottom:4px}
        .modal-sub{font-size:12px;color:var(--muted);margin-bottom:16px}
        .modal-input{width:100%;background:var(--bg2);border:1.5px solid var(--border);border-radius:10px;padding:13px 14px;color:var(--text);font-family:var(--mono);font-size:20px;font-weight:700;outline:none;transition:border-color .2s}
        ...
        .m-confirm{padding:11px;background:linear-gradient(135deg,#6366f1,#8b5cf6);border:none;border-radius:10px;color:#fff;font-family:var(--font);font-size:13px;font-weight:800;cursor:pointer;transition:opacity .15s;box-shadow:0 3px 12px rgba(99,102,241,.35)}
        .m-confirm:hover{opacity:.9}
 
        /* RECEIPT */
        .receipt-box{background:#0f1117;border:1px solid rgba(255,255,255,.08);border-radius:20px;padding:24px;width:100%;max-width:320px;animation:modalUp .25s cubic-bezier(.34,1.56,.64,1);box-shadow:0 24px 64px rgba(0,0,0,.7)}
        ...
        .r-btns{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:18px}
        .r-print{padding:13px;background:rgba(99,102,241,.15);border:1px solid rgba(99,102,241,.3);border-radius:11px;color:#818cf8;font-family:var(--font);font-size:13px;font-weight:800;cursor:pointer;transition:all .15s}
        .r-print:hover{background:rgba(99,102,241,.25)}
        .r-close{padding:13px;background:linear-gradient(135deg,#6366f1,#8b5cf6);color:#fff;border:none;border-radius:11px;font-family:var(--font);font-size:13px;font-weight:800;cursor:pointer;box-shadow:0 4px 16px rgba(99,102,241,.35);transition:opacity .15s}
              ...
              <div className="pay-section">
                ...
                {payMethod==="cash"&&(
                  <div className="cash-wrap">
                    <input className="num-input" type="number" placeholder="Berilgan pul (so'm)"
