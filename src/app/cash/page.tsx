...
const CAT_COLORS = ["#6366f1","#10b981","#f59e0b","#ef4444","#8b5cf6","#06b6d4","#ec4899","#84cc16","#14b8a6","#f97316"];
...
const generateReceiptHTML = (sale: any): string => {
  ...
  <style>@page{size:80mm auto;margin:4mm}*{margin:0;padding:0;box-sizing:border-box}
  body{font-family:'Courier New',monospace;font-size:12px;color:#111;width:72mm;padding:4mm}
  .center{text-align:center}.bold{font-weight:700}.dashed{border-top:1.5px dashed #555;margin:6px 0}
  table{width:100%;border-collapse:collapse}th{font-size:9px;text-align:left;padding:4px 2px;border-bottom:2px solid #111;color:#444}
  .total-row{display:flex;justify-content:space-between;padding:3px 0;font-size:11px}
  .total-row.main{font-size:15px;font-weight:900;border-top:2px solid #111;padding-top:6px}
  ...
  @media print{body{width:100%;padding:0}}</style></head>
  <body>
  <div class="center"><div style="font-size:18px;font-weight:900">OMBORCHI.UZ</div>
  <div style="font-size:9px;color:#555">${date}</div>
  ${sale.warehouseName?`<div style="font-size:9px;font-weight:700">🏪 ${sale.warehouseName}</div>`:""}
  <div style="font-size:9px">Kassir: <strong>${sale.cashierName}</strong></div>
  <div style="font-size:9px">Chek: <strong>#${sale.id?.slice(-10)||"—"}</strong></div></div>
  <div class="dashed"></div>
  <table><thead><tr><th>Mahsulot</th><th style="text-align:center">Dona</th><th style="text-align:right">Narx</th><th style="text-align:right">Summa</th></tr></thead>
  <tbody>${rows}</tbody></table>
  <div class="total-row main"><span>JAMI:</span><span>${new Intl.NumberFormat("uz-UZ").format(sale.total)} so'm</span></div>
  <div class="pay-info">
  <div class="pay-row"><span>To'lov:</span><span class="bold">${PAY_LABELS[sale.paymentMethod]||sale.paymentMethod}</span></div>
  ${sale.paymentMethod==="cash"?`<div class="pay-row"><span>Berildi:</span><span>${new Intl.NumberFormat("uz-UZ").format(sale.cashGiven||0)} so'm</span></div>
  ...
  <div class="footer"><div>Xarid uchun rahmat! 🙏</div><div>www.omborchi.uz</div></div>
...
const downloadPDF = (sale: any) => {
  ...
  if (!docEl) return;
  docEl.open(); docEl.write(generateReceiptHTML(sale)); docEl.close();
  setTimeout(() => { try { iframe.contentWindow?.print(); } catch {} setTimeout(() => document.body.removeChild(iframe), 3000); }, 500);
};
...
export default function CashPage() {
    ...
    try {
        ...
        try {
          ...
          if (!snap.empty) {
            rawData = snap.docs.map(d => {
              const r = d.data() as any;
              const name = r.name||r.nomi||r.nom||r.title||r.mahsulot||r.tovar||`Mahsulot #${d.id.slice(-4)}`;
              const price = Number(r.salePrice||r.price||r.narx||r.baho||r.sotishNarxi||r.selling_price||r.sellingPrice||r.cost||0);
              const category = r.category||r.kategoriya||r.cat||r.tip||r.tur||r.type||"";
              return {
                ...r, id: d.id, name, price, salePrice: price, category, kategoriya: category,
                stock: r.stock, qoldiq: r.qoldiq, miqdor: r.miqdor,
      ...
      if (wh && rawData.length > 0) {
        ...
        for (const f of whFields) {
          ...
          if (filtered.length > 0) { rawData = filtered; break; }
        }
      }
  ...
  const selectWarehouse = (wh: Warehouse) => {
    ...
    setActiveWh(wh); setShowWhModal(false);
  ...
  const addToCart = (p: Product) => {
    ...
    setCart(prev => {
      ...
      if (ex) {
        if (stock < 9999 && ex.quantity >= stock) { showToast("Omborda yetarli emas!", "err"); return prev; }
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      showToast("✓ Sotuv amalga oshdi!", "ok");
      setShowReceipt(true);
      fetchProducts(activeWh); fetchInventory(activeWh); fetchSales(); fetchReport();
  ...
  const css = `
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .tab-btn.active { background: var(--sur2); color: var(--text); box-shadow: 0 1px 4px rgba(0,0,0,.3); }
    .topbar-right { margin-left: auto; display: flex; align-items: center; gap: 8px; }
    .wh-pill {
      display: flex; align-items: center; gap: 5px;
      padding: 5px 11px 5px 8px;
      background: var(--accbg); border: 1px solid rgba(99,102,241,.2);
      border-radius: 18px; cursor: pointer; transition: all .15s;
    }
    .wh-pill:hover { border-color: rgba(99,102,241,.4); }
    .wh-dot { width: 6px; height: 6px; border-radius: 50%; background: var(--accent); box-shadow: 0 0 4px var(--accent); flex-shrink: 0; }
    .wh-name { font-size: 11px; font-weight: 700; color: var(--acc2); max-width: 120px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .user-chip {
    ...
    .toast {
      ...
      z-index: 9999; padding: 9px 20px; border-radius: 28px;
      font-size: 11.5px; font-weight: 700; white-space: nowrap;
      animation: toastIn .22s cubic-bezier(.34,1.56,.64,1);
      box-shadow: 0 6px 24px rgba(0,0,0,.35);
    }
    .toast.ok   { background: rgba(16,185,129,.14); color: #34d399; border: 1px solid rgba(16,185,129,.22); }
    .toast.err  { background: rgba(239,68,68,.14);  color: #f87171; border: 1px solid rgba(239,68,68,.22); }
    .toast.info { background: rgba(99,102,241,.14); color: #818cf8; border: 1px solid rgba(99,102,241,.22); }
    @keyframes toastIn {
      from { opacity:0; transform: translateX(-50%) translateY(-8px) scale(.93); }
    ...
    .wh-bar {
      ...
      padding: 8px 12px;
      background: var(--accbg); border: 1px solid rgba(99,102,241,.15);
      border-radius: var(--rs); cursor: pointer; transition: all .15s;
    }
    .wh-bar:hover { border-color: rgba(99,102,241,.35); }
    .wh-bar-lbl  { font-size: 9px; font-weight: 700; color: var(--muted); text-transform: uppercase; letter-spacing: .6px; }
    .wh-bar-name { font-size: 11.5px; font-weight: 700; color: var(--acc2); flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .wh-bar-cnt  { font-family: var(--mono); font-size: 9.5px; font-weight: 600; color: var(--acc2); background: rgba(99,102,241,.14); padding: 2px 7px; border-radius: 18px; }
    .wh-bar-btn  { font-size: 9.5px; font-weight: 700; color: var(--accent); padding: 3px 8px; border-radius: 5px; border: 1px solid rgba(99,102,241,.22); background: rgba(99,102,241,.07); }
    ...
    .search-inp  {
      ...
      border-radius: var(--rs); padding: 9px 11px 9px 33px;
      color: var(--text); font-family: var(--font); font-size: 13px; outline: none; transition: all .15s;
    }
    .search-inp::placeholder { color: var(--muted); }
    .search-inp:focus { border-color: rgba(99,102,241,.45); background: var(--sur2); }
    ...
    .cat-pill.active { background: var(--accent); border-color: var(--accent); color: #fff; box-shadow: 0 2px 9px rgba(99,102,241,.3); }
    ...
    .prod-card {
      ...
      display: flex;
      flex-direction: column;
      min-height: 215px;       /* ← KEY FIX: cards never collapse */
      padding: 0;
      overflow: hidden;
      position: relative;
      ...
      /* performance hint for 1500+ cards */
    ...
    .prod-card:hover {
      ...
      box-shadow: 0 10px 30px rgba(0,0,0,.45), 0 0 0 1px rgba(99,102,241,.12);
    }
    .prod-card:active { transform: scale(.97); transition-duration: .07s; }
    .prod-card:disabled {
      opacity: .4; cursor: not-allowed;
      transform: none !important; filter: grayscale(.5);
    }
    ...
    .prod-img-wrap {
      ...
      overflow: hidden;
      border-bottom: 1px solid var(--border);
      margin-top: 3px;           /* space for cat-bar */
    ...
    .prod-price.no-price { color: var(--muted) !important; -webkit-text-fill-color: var(--muted) !important; }
 
    .prod-foot { display: flex; align-items: center; justify-content: space-between; margin-top: auto; padding-top: 5px; }
    ...
    .prod-cat-label {
      font-size: 8.5px; font-weight: 700; color: var(--muted) !important;
      -webkit-text-fill-color: var(--muted) !important;
      overflow: hidden; text-overflow: ellipsis; white-space: nowrap; max-width: 55px;
    ...
    .prod-badge {
      ...
      display: flex; align-items: center; justify-content: center;
      box-shadow: 0 2px 9px rgba(99,102,241,.55);
    ...
    .empty-state {
      ...
      color: var(--muted); padding: 70px 20px; text-align: center; gap: 10px;
    }
    ...
    .cart-title-row { display: flex; align-items: center; gap: 7px; }
    .cart-title { font-size: 13.5px; font-weight: 800; }
    .cart-cnt   { min-width: 19px; height: 19px; padding: 0 4px; background: var(--accent); border-radius: 9px; font-size: 9.5px; font-weight: 800; color: #fff; display: flex; align-items: center; justify-content: center; }
    .cart-clear { background: rgba(239,68,68,.09); border: 1px solid rgba(239,68,68,.18); border-radius: 6px; padding: 4px 8px; color: #f87171; font-family: var(--font); font-size: 9.5px; font-weight: 700; cursor: pointer; transition: background .12s; }
    .cart-clear:hover { background: rgba(239,68,68,.18); }
    .cart-list   { flex: 1; overflow-y: auto; min-height: 0; padding: 8px; display: flex; flex-direction: column; gap: 5px; }
    .cart-empty  { flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 9px; color: var(--muted); }
    .cart-empty-ico { font-size: 36px; opacity: .25; }
    .cart-empty-txt { font-size: 11.5px; font-weight: 600; }
    .cart-item { background: var(--sur2); border: 1px solid var(--border); border-radius: var(--rs); padding: 9px; display: flex; flex-direction: column; gap: 7px; animation: itemIn .16s ease; }
    ...
    .ci-name { font-size: 11px; font-weight: 700; color: var(--text); flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .ci-del  { width: 18px; height: 18px; border: none; border-radius: 4px; background: rgba(239,68,68,.09); color: #f87171; font-size: 9px; cursor: pointer; flex-shrink: 0; display: flex; align-items: center; justify-content: center; transition: background .1s; }
    .ci-del:hover { background: rgba(239,68,68,.2); }
    .ci-price-btn { width: 100%; background: var(--sur3); border: 1px solid var(--border); border-radius: 6px; padding: 5px 8px; display: flex; align-items: center; gap: 5px; cursor: pointer; transition: border-color .12s; }
    .ci-price-btn:hover { border-color: rgba(99,102,241,.35); }
    ...
    .ci-edit  { margin-left: auto; font-size: 9.5px; color: var(--muted); }
    .ci-bottom { display: flex; align-items: center; justify-content: space-between; }
    .qty-ctrl  { display: flex; align-items: center; gap: 5px; }
    .qty-btn   { width: 24px; height: 24px; border: 1px solid var(--border); border-radius: 6px; background: var(--sur3); color: var(--text); font-size: 14px; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all .1s; font-family: var(--mono); }
    .qty-btn:hover { background: var(--accbg); border-color: rgba(99,102,241,.35); color: var(--acc2); }
    .qty-val { font-family: var(--mono); font-size: 13px; font-weight: 700; width: 22px; text-align: center; }
    .ci-sub  { font-family: var(--mono); font-size: 11px; font-weight: 700; color: var(--text); }
 
    /* payment section */
    .pay-section { padding: 11px; flex-shrink: 0; border-top: 1px solid var(--border); display: flex; flex-direction: column; gap: 9px; }
    ...
    .total-amt { font-family: var(--mono); font-size: 21px; font-weight: 800; }
    .pay-methods { display: grid; grid-template-columns: repeat(3,1fr); gap: 5px; }
    .pay-btn { padding: 8px 3px; border-radius: 8px; border: 1.5px solid var(--border); background: var(--sur2); color: var(--muted); font-family: var(--font); font-size: 10px; font-weight: 700; cursor: pointer; transition: all .15s; display: flex; flex-direction: column; align-items: center; gap: 3px; }
    .pay-btn:hover { color: var(--text2); border-color: var(--bord2); }
    .pay-btn-ico { font-size: 14px; }
    .cash-wrap { display: flex; flex-direction: column; gap: 5px; }
    .num-inp { width: 100%; background: var(--sur2); border: 1.5px solid var(--border); border-radius: 8px; padding: 9px 11px; color: var(--text); font-family: var(--mono); font-size: 14px; font-weight: 700; outline: none; transition: border-color .15s; }
    .num-inp:focus { border-color: rgba(99,102,241,.45); }
    .num-inp::placeholder { color: var(--muted); font-family: var(--font); font-size: 10.5px; font-weight: 400; }
    .change-row { display: flex; justify-content: space-between; align-items: center; padding: 5px 9px; border-radius: 7px; font-family: var(--mono); font-size: 11px; font-weight: 700; }
    .change-row.pos { background: rgba(16,185,129,.09); color: #34d399; border: 1px solid rgba(16,185,129,.18); }
    .change-row.neg { background: rgba(239,68,68,.09); color: #f87171; border: 1px solid rgba(239,68,68,.18); }
    .quick-grid { display: grid; grid-template-columns: repeat(3,1fr); gap: 4px; }
    .quick-btn { padding: 4px 3px; background: var(--sur3); border: 1px solid var(--border); border-radius: 5px; color: var(--muted); font-family: var(--mono); font-size: 9.5px; font-weight: 700; cursor: pointer; transition: all .1s; text-align: center; }
    .quick-btn:hover { border-color: rgba(99,102,241,.3); color: var(--acc2); background: var(--accbg); }
    .checkout-btn { width: 100%; padding: 12px; border-radius: 10px; border: none; background: linear-gradient(135deg,#6366f1,#8b5cf6); color: #fff; font-family: var(--font); font-size: 13px; font-weight: 800; cursor: pointer; transition: all .15s; box-shadow: 0 4px 16px rgba(99,102,241,.3); }
    .checkout-btn:hover:not(:disabled) { transform: translateY(-2px); box-shadow: 0 7px 24px rgba(99,102,241,.42); }
    .checkout-btn:active:not(:disabled) { transform: translateY(0); transition-duration: .07s; }
    .checkout-btn:disabled { background: var(--sur3); color: var(--muted); cursor: not-allowed; box-shadow: none; }
 
    /* ═══ PAGES (history/payments/report) ═══ */
    .page { flex: 1; overflow-y: auto; padding: 20px; max-width: 1040px; width: 100%; margin: 0 auto; }
    ...
    .refresh-btn { padding: 6px 13px; background: var(--sur2); border: 1px solid var(--bord2); border-radius: var(--rs); color: var(--muted); font-family: var(--font); font-size: 10.5px; font-weight: 700; cursor: pointer; transition: all .12s; }
    .refresh-btn:hover { color: var(--acc2); border-color: rgba(99,102,241,.35); }
    .kpi-grid { display: grid; grid-template-columns: repeat(auto-fill,minmax(150px,1fr)); gap: 8px; margin-bottom: 18px; }
    .kpi-card { background: var(--surface); border: 1px solid var(--border); border-radius: var(--r); padding: 14px; }
    .kpi-lbl  { font-size: 9px; font-weight: 700; color: var(--muted); text-transform: uppercase; letter-spacing: .5px; margin-bottom: 6px; }
    .kpi-val  { font-family: var(--mono); font-size: 16px; font-weight: 700; }
    .sale-card { background: var(--surface); border: 1px solid var(--border); border-radius: var(--r); padding: 13px; margin-bottom: 8px; transition: border-color .12s; animation: cardIn .22s ease; }
    ...
    .sc-id     { font-family: var(--mono); font-size: 9px; color: var(--muted); }
    .sc-date   { font-size: 10px; color: var(--muted); margin-top: 2px; }
    .sc-cashier{ font-size: 10px; font-weight: 600; color: var(--text2); margin-top: 2px; }
    .sc-total  { font-family: var(--mono); font-size: 14px; font-weight: 700; color: var(--acc2); }
    .pay-badge { display: inline-flex; align-items: center; gap: 3px; padding: 2px 8px; border-radius: 18px; font-size: 9px; font-weight: 700; border: 1px solid; font-family: var(--font); margin-top: 3px; }
    .sc-items  { border-top: 1px solid var(--border); padding-top: 8px; display: flex; flex-direction: column; gap: 4px; }
    .sc-item-row { display: flex; justify-content: space-between; font-size: 10.5px; color: var(--text2); font-family: var(--mono); }
    .pay-sum-grid { display: grid; grid-template-columns: repeat(3,1fr); gap: 9px; margin-bottom: 18px; }
    .pay-sum-card { border-radius: var(--r); border: 1px solid; padding: 14px; }
    .pay-sum-lbl  { font-size: 9.5px; font-weight: 700; text-transform: uppercase; letter-spacing: .5px; margin-bottom: 6px; }
    .pay-sum-val  { font-family: var(--mono); font-size: 14px; font-weight: 800; }
    .bar-card  { background: var(--surface); border: 1px solid var(--border); border-radius: var(--r); padding: 16px; margin-bottom: 10px; }
    .bar-title { font-size: 11.5px; font-weight: 700; margin-bottom: 12px; }
    .bar-track { height: 8px; background: var(--bg2); border-radius: 4px; overflow: hidden; display: flex; gap: 2px; }
    .bar-seg   { height: 100%; border-radius: 3px; transition: width .6s ease; }
    .bar-legend { display: flex; gap: 13px; margin-top: 9px; flex-wrap: wrap; }
    .bar-legend-item { display: flex; align-items: center; gap: 4px; font-size: 10.5px; color: var(--text2); font-weight: 600; }
    .bar-dot   { width: 6px; height: 6px; border-radius: 50%; }
    .top-row   { display: flex; align-items: center; gap: 10px; margin-bottom: 10px; }
    .top-rank  { font-family: var(--mono); font-size: 10.5px; font-weight: 700; color: var(--muted); width: 18px; flex-shrink: 0; }
    .top-info  { flex: 1; }
    .top-name-row { display: flex; justify-content: space-between; font-size: 11.5px; font-weight: 700; margin-bottom: 4px; }
    .top-qty   { font-family: var(--mono); font-size: 9.5px; color: var(--muted); font-weight: 400; }
    ...
    .top-revenue { font-family: var(--mono); font-size: 9.5px; color: var(--acc2); margin-top: 3px; }
 
    /* ═══ OVERLAYS / MODALS ═══ */
    .overlay {
      position: fixed; inset: 0; background: rgba(0,0,0,.7);
      display: flex; align-items: center; justify-content: center;
      z-index: 200; padding: 14px; backdrop-filter: blur(12px);
      animation: fadeIn .15s ease;
    }
    @keyframes fadeIn { from{opacity:0} to{opacity:1} }
    @keyframes modalUp { from{transform:translateY(16px) scale(.95);opacity:0} to{transform:translateY(0) scale(1);opacity:1} }
    ...
    .wh-modal-hdr  { padding: 18px 16px 14px; border-bottom: 1px solid var(--border); }
    .wh-modal-ttl  { font-size: 16px; font-weight: 800; letter-spacing: -.3px; margin-bottom: 2px; }
    .wh-modal-sub  { font-size: 10.5px; color: var(--muted); }
    .wh-list       { padding: 9px; display: flex; flex-direction: column; gap: 5px; max-height: 340px; overflow-y: auto; }
    .wh-item       { display: flex; align-items: center; gap: 11px; padding: 11px 13px; background: var(--sur2); border: 1.5px solid var(--border); border-radius: var(--r); cursor: pointer; transition: all .15s; text-align: left; width: 100%; font-family: var(--font); }
    .wh-item:hover { border-color: rgba(99,102,241,.35); background: var(--accbg); }
    .wh-item.sel   { border-color: var(--accent); background: rgba(99,102,241,.1); }
    .wh-item-ico   { width: 40px; height: 40px; border-radius: 9px; background: var(--sur3); border: 1px solid var(--bord2); display: flex; align-items: center; justify-content: center; font-size: 17px; flex-shrink: 0; }
    .wh-item-name  { font-size: 12.5px; font-weight: 700; color: var(--text); }
    .wh-item.sel .wh-item-name { color: var(--acc2); }
    .wh-item-addr  { font-size: 9.5px; color: var(--muted); margin-top: 2px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .wh-check      { width: 19px; height: 19px; border-radius: 50%; border: 2px solid var(--bord2); display: flex; align-items: center; justify-content: center; font-size: 9px; flex-shrink: 0; transition: all .15s; }
    .wh-item.sel .wh-check { background: var(--accent); border-color: var(--accent); color: #fff; box-shadow: 0 2px 8px rgba(99,102,241,.4); }
    .wh-modal-foot { padding: 9px 16px; border-top: 1px solid var(--border); }
    .cancel-btn    { width: 100%; padding: 9px; background: var(--sur2); border: 1.5px solid var(--border); border-radius: 8px; color: var(--muted); font-family: var(--font); font-size: 11.5px; font-weight: 700; cursor: pointer; transition: all .12s; }
    .cancel-btn:hover { color: var(--text); border-color: var(--bord2); }
 
    .modal-box  { background: var(--surface); border: 1px solid var(--bord2); border-radius: 16px; padding: 20px; width: 100%; max-width: 300px; animation: modalUp .2s cubic-bezier(.34,1.56,.64,1); box-shadow: 0 24px 60px rgba(0,0,0,.55); }
    .modal-ttl  { font-size: 15px; font-weight: 800; letter-spacing: -.3px; margin-bottom: 2px; }
    .modal-sub  { font-size: 10.5px; color: var(--muted); margin-bottom: 13px; }
    .modal-inp  { width: 100%; background: var(--bg2); border: 1.5px solid var(--border); border-radius: 8px; padding: 11px 12px; color: var(--text); font-family: var(--mono); font-size: 17px; font-weight: 700; outline: none; transition: border-color .15s; }
    .modal-inp:focus { border-color: rgba(99,102,241,.45); background: var(--sur2); }
    .modal-inp::placeholder { color: var(--muted); font-family: var(--font); font-size: 11px; font-weight: 400; }
    .modal-btns { display: grid; grid-template-columns: 1fr 1fr; gap: 6px; margin-top: 13px; }
    .m-cancel   { padding: 9px; background: var(--sur2); border: 1.5px solid var(--border); border-radius: 8px; color: var(--muted); font-family: var(--font); font-size: 11.5px; font-weight: 700; cursor: pointer; }
    .m-confirm  { padding: 9px; background: linear-gradient(135deg,#6366f1,#8b5cf6); border: none; border-radius: 8px; color: #fff; font-family: var(--font); font-size: 11.5px; font-weight: 800; cursor: pointer; box-shadow: 0 3px 9px rgba(99,102,241,.32); }
 
    .receipt-box { background: #0a0c18; border: 1px solid rgba(255,255,255,.08); border-radius: 18px; padding: 20px; width: 100%; max-width: 300px; animation: modalUp .2s cubic-bezier(.34,1.56,.64,1); box-shadow: 0 24px 60px rgba(0,0,0,.75); }
    .r-hdr { text-align: center; margin-bottom: 14px; }
    .r-ico  { font-size: 38px; margin-bottom: 6px; }
    .r-ttl  { font-size: 16px; font-weight: 800; letter-spacing: -.3px; }
    .r-id   { font-family: var(--mono); font-size: 9px; color: var(--muted); margin-top: 2px; }
    .r-cashier { font-size: 9.5px; color: var(--muted); margin-top: 2px; }
    .r-wh   { font-size: 9.5px; color: var(--acc2); margin-top: 2px; font-weight: 700; }
    .r-div  { border: none; border-top: 1.5px dashed rgba(255,255,255,.07); margin: 10px 0; }
    .r-row  { display: flex; justify-content: space-between; font-size: 10.5px; color: var(--text2); margin-bottom: 4px; font-family: var(--mono); }
    .r-row.bold  { font-weight: 800; font-size: 14px; color: var(--text); }
    .r-row.muted { color: var(--muted); }
    .r-thanks { text-align: center; font-size: 9.5px; color: var(--muted); margin-top: 11px; }
    .r-btns { display: flex; flex-direction: column; gap: 5px; margin-top: 14px; }
    .r-btn-row { display: grid; grid-template-columns: 1fr 1fr; gap: 5px; }
    .r-btn { padding: 10px; border-radius: 9px; font-family: var(--font); font-size: 11.5px; font-weight: 800; cursor: pointer; transition: all .13s; display: flex; align-items: center; justify-content: center; gap: 4px; }
    .r-btn-print { background: rgba(99,102,241,.13); border: 1px solid rgba(99,102,241,.27); color: #818cf8; }
    .r-btn-print:hover { background: rgba(99,102,241,.22); }
    .r-btn-pdf   { background: rgba(239,68,68,.1); border: 1px solid rgba(239,68,68,.22); color: #f87171; }
    .r-btn-pdf:hover { background: rgba(239,68,68,.2); }
    .r-btn-close { background: linear-gradient(135deg,#6366f1,#8b5cf6); border: none; color: #fff; box-shadow: 0 3px 12px rgba(99,102,241,.32); }
          ...
          <div className="pos-layout">
            ...
            <div className="products-panel">
              <div className="prod-toolbar">
                ...
                <div className="search-wrap">
                  ...
                  <input className="search-inp" placeholder="Mahsulot nomi yoki kodi…" value={search} onChange={e=>setSearch(e.target.value)}/>
                </div>
                {categories.length>1 && (
                  <div className="cats">
                    {categories.map(c=>(
              ...
              <div className="products-grid">
                ...
                {/* SKELETON — explicit height, never collapses */}
                {productsLoading ? (
                  Array.from({length:20}).map((_,i)=>(
                    <div key={i} className="skel-card">
                      <div className="skel-img-area skel-line"/>
                      <div className="skel-text-area">
                        <div className="skel-line" style={{height:13,borderRadius:4}}/>
                        <div className="skel-line" style={{height:12,width:"65%",borderRadius:4}}/>
                        <div className="skel-line" style={{height:9,width:"42%",borderRadius:4,marginTop:4}}/>
                      </div>
                    </div>
                  ))
                  ...
                  <div className="empty-state">
                    ...
                    {allProducts.length===0 && (
                      <div className="empty-hint">Firebase "products" collectionini tekshiring</div>
                    )}
                  </div>
                  ...
                  filtered.map((p, idx)=>{
                    ...
                    return (
                      <button
                        key={p.id}
                        className="prod-card"
                        style={{animationDelay:`${Math.min(idx*.016,.45)}s`}}
                        onClick={()=>addToCart(p)}
                        disabled={noStock}
                        title={p.name}
                      >
                        ...
                        {/* image */}
                        <div className={`prod-img-wrap${noStock?" no-stock":""}`}>
                          {p.imageUrl
                        ...
                        <div className="prod-text">
                          ...
                          <span className={`prod-price${price===0?" no-price":""}`}>
                            {price>0 ? fmt(price) : "Narx kiritilmagan"}
                          </span>
                          <div className="prod-foot">
                            ...
                            {cat && cat!=="Boshqa" && (
                              <span className="prod-cat-label" style={{color:color+"bb !important"}}>{cat}</span>
                            )}
                          </div>
                        </div>
                      </button>
            ...
            <div className="cart-panel">
              ...
              <div className="pay-section">
                ...
                >
                  {loading?"⏳ Amalga oshirilmoqda…"
                  :cart.length===0?"Savat bo'sh"
                  :`✓ To'lash — ${fmt(total)}`}
                </button>
              </div>
            </div>
                ...
                <div className="sc-top">
                  <div>
                    ...
                    <div style={{display:"flex",gap:5,flexWrap:"wrap",marginTop:4}}>
                      ...
                      {sale.warehouseName&&<span style={{fontSize:9,color:"#818cf8",background:"rgba(99,102,241,.09)",border:"1px solid rgba(99,102,241,.18)",borderRadius:18,padding:"2px 7px",fontWeight:700}}>🏪 {sale.warehouseName}</span>}
                    </div>
                  </div>
                  <div style={{textAlign:"right"}}>
                    <div className="sc-total">{fmt(sale.total)}</div>
                    <div style={{display:"flex",gap:4,marginTop:5,justifyContent:"flex-end"}}>
                      <button onClick={()=>printBrowser(sale)} style={{padding:"3px 8px",background:"rgba(99,102,241,.09)",border:"1px solid rgba(99,102,241,.22)",borderRadius:5,color:"#818cf8",fontSize:9.5,fontWeight:700,cursor:"pointer"}}>🖨️ Chek</button>
                      <button onClick={()=>downloadPDF(sale)} style={{padding:"3px 8px",background:"rgba(239,68,68,.09)",border:"1px solid rgba(239,68,68,.22)",borderRadius:5,color:"#f87171",fontSize:9.5,fontWeight:700,cursor:"pointer"}}>📄 PDF</button>
                    </div>
                  </div>
                </div>
            ...
            {sales.map(sale=>(
              <div key={sale.id} className="sale-card" style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                <div>
                  <div className="sc-id">#{sale.id.slice(-10)}</div>
                  <div className="sc-date">{fmtDate(sale.createdAt)}</div>
                  <div style={{fontSize:10,color:"var(--text2)",marginTop:2}}>{sale.items?.length} ta mahsulot</div>
                  {sale.warehouseName&&<div style={{fontSize:9,color:"#818cf8",marginTop:2,fontWeight:700}}>🏪 {sale.warehouseName}</div>}
                  {sale.paymentMethod==="cash"&&sale.change!==undefined&&(
                    <div style={{fontSize:9,color:"var(--muted)",marginTop:2,fontFamily:"var(--mono)"}}>Qaytim: {fmt(sale.change)}</div>
                  )}
                </div>
                <div style={{textAlign:"right"}}>
                  <div className="sc-total">{fmt(sale.total)}</div>
                  ...
                  <div style={{display:"flex",gap:4,marginTop:4,justifyContent:"flex-end"}}>
                    <button onClick={()=>printBrowser(sale)} style={{padding:"3px 7px",background:"rgba(99,102,241,.09)",border:"1px solid rgba(99,102,241,.22)",borderRadius:5,color:"#818cf8",fontSize:9,fontWeight:700,cursor:"pointer"}}>🖨️</button>
                    <button onClick={()=>downloadPDF(sale)} style={{padding:"3px 7px",background:"rgba(239,68,68,.09)",border:"1px solid rgba(239,68,68,.22)",borderRadius:5,color:"#f87171",fontSize:9,fontWeight:700,cursor:"pointer"}}>📄</button>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="overlay" onClick={e=>{if(e.target===e.currentTarget)setShowWhModal(false)}}>
            <div className="wh-modal">
              ...
              <div className="wh-list">
                {warehouses.length===0?(
                  <div className="empty-state" style={{padding:"24px 0",gridColumn:"unset"}}>
                    <div className="empty-ico">🏪</div>
        ...
        {customPriceItem&&(
          <div className="overlay" onClick={e=>{if(e.target===e.currentTarget){setCustomPriceItem(null);setTempPrice("");}}}>
            <div className="modal-box">
              ...
              <div className="modal-sub">{customPriceItem.name} · Asl: {fmt(getPrice(customPriceItem))}</div>
