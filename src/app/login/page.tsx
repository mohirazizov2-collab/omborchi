...
import { Loader2, Lock, Mail, Globe, AlertCircle, Warehouse, ArrowRight, Box, Package, Truck, Layers, ShieldCheck, Eye, EyeOff } from "lucide-react";
...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errors: Record<string, string> = {
        ...
        "auth/too-many-requests": "Urinishlar soni oshib ketdi. Birozdan song qayta urinib koring.",
        "auth/network-request-failed": "Internet aloqasini tekshirib, qayta urinib koring.",
      };
      setError(errors[err?.code] || "Tizimga kirishda xatolik yuz berdi. Qayta urinib koring.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePasswordReset = async () => {
    ...
    if (!normalizedEmail) {
      setError("Parolni tiklash uchun emailingizni kiriting.");
      return;
    }
    ...
    try {
      ...
      toast({
        title: "Tiklash havolasi yuborildi",
        description: "Emailingizni tekshiring va yangi parol ornating.",
      });
    } catch (err: any) {
      setError(err?.code === "auth/user-not-found"
        ? "Bu email bilan foydalanuvchi topilmadi."
        : "Tiklash havolasini yuborib bolmadi. Qayta urinib koring.");
    } finally {
      setLoading(false);
    }
  };
            ...
            <form onSubmit={handleLogin}>
              <CardContent className="space-y-6 pt-8 px-10">
                ...
                <div className="space-y-2.5">
                  <Label htmlFor="email" className="text-muted-foreground/60 text-[10px] font-black uppercase tracking-widest pl-2">{t.auth.emailLabel}</Label>
                ...
                <div className="space-y-2.5">
                  <Label htmlFor="password" className="text-muted-foreground/60 text-[10px] font-black uppercase tracking-widest pl-2">{t.auth.passwordLabel}</Label>
                  <div className="relative group">
                    ...
                    <input
                      ...
                      className="flex h-14 w-full pl-12 pr-12 rounded-[1.5rem] bg-background/50 border border-border/40 text-foreground placeholder:text-muted-foreground/20 focus:outline-none focus:ring-2 focus:ring-primary/40 text-sm transition-all font-medium"
                    ...
                    <button
                      ...
                      className="absolute right-4 top-1/2 -translate-y-1/2 rounded-xl p-2 text-muted-foreground/50 transition-colors hover:bg-muted hover:text-primary"
                    >
                ...
                <div className="flex justify-end px-2 -mt-2">
                  <button
                    ...
                    disabled={loading}
                    className="text-[10px] font-black uppercase tracking-wider text-primary transition-opacity hover:underline disabled:cursor-not-allowed disabled:opacity-50"
                  >
              ...
              <CardFooter className="pt-6 pb-12 px-10">
                <Button 
                  ...
                  disabled={loading}
                >
                  {loading ? <><Loader2 className="w-5 h-5 animate-spin" /> {t.auth.loggingIn}</> : <>{t.auth.loginButton} <ArrowRight className="w-4 h-4 ml-2" /></>}
                </Button>
              </CardFooter>
            </form>
          </Card>
