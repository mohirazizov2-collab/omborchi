...
export default function ProductsPage() {
  ...
  // ✅ TO'G'IRLANGAN: async/await bilan handleSave
  const handleSave = async () => {
    ...
    try {
      ...
      console.error("Firestore xatolik:", error);
      ...
      toast({
        variant: "destructive",
        title: isNetworkError ? "Internet aloqasi yo'q" : "Xatolik yuz berdi",
  ...
  // ✅ TO'G'IRLANGAN: async/await bilan handleDelete
  ...
  // ✅ TO'G'IRLANGAN: async/await bilan handleBulkDelete
  ...
  // ✅ TO'G'IRLANGAN: async/await bilan handleCreateCategory
  ...
  // ✅ TO'G'IRLANGAN: async/await bilan handleDeleteCategory
  const handleDeleteCategory = async (id: string, name: string) => {
    ...
    try {
      ...
      console.error("Guruh o'chirishda xatolik:", error);
      toast({
        variant: "destructive",
        title: "Guruh o'chirishda xatolik",
        description: error?.message || "Qayta urinib ko'ring",
      });
    }
  };
        ...
        <div className="px-6 pt-5 pb-4 border-b border-border/20 bg-card/30 flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {filteredProducts.length > 0 && (
              <Button
                variant="outline" size="sm"
                className="h-8 px-3 text-xs rounded-lg gap-1.5 text-emerald-600 border-emerald-600/20"
            ...
            <div className="p-2 space-y-0.5">
              ...
              >
                ...
                <span className={cn(
                  ...
                  selectedCategoryId === "all" ? "text-white/70" : "text-muted-foreground"
                )}>
                  {products?.length || 0}
                </span>
              </button>
              ...
              >
                ...
                <span className={cn(
                  "text-[10px] font-black shrink-0",
                  selectedCategoryId === "none" ? "text-white/70" : "text-muted-foreground"
                )}>
              ...
              {categories?.map((cat) => (
                <div key={cat.id} className="group relative">
                  ...
                  >
                    ...
                    <span className={cn(
                      "text-[10px] font-black shrink-0",
                      selectedCategoryId === cat.id ? "text-white/70" : "text-muted-foreground"
                    )}>
                      {catCounts[cat.id] || 0}
                    </span>
                  </button>
                ...
                <table className="w-full text-sm text-left">
                  <thead className="bg-muted/20 border-b border-border/20 sticky top-0 z-10">
                    <tr>
                      ...
                      <th
                        className="px-3 py-3 text-[10px] font-black uppercase text-muted-foreground cursor-pointer hover:text-foreground transition-colors w-24"
                        onClick={() => handleSort("sku")}
                      >
                        <span className="flex items-center gap-1">Kod <SortIcon field="sku" /></span>
                      </th>
                      <th
                        className="px-3 py-3 text-[10px] font-black uppercase text-muted-foreground cursor-pointer hover:text-foreground"
                      ...
                      <th
                        className="px-3 py-3 text-[10px] font-black uppercase text-muted-foreground text-center cursor-pointer hover:text-foreground w-28"
                        onClick={() => handleSort("stock")}
                      >
                        <span className="flex items-center gap-1 justify-center">Zaxira <SortIcon field="stock" /></span>
                      </th>
                      <th
                        className="px-3 py-3 text-[10px] font-black uppercase text-muted-foreground text-right cursor-pointer hover:text-foreground w-32"
                      ...
                      {canEdit && (
                        <th className="px-3 py-3 w-20 text-[10px] font-black uppercase text-muted-foreground text-center">
                          Amallar
                        </th>
                      )}
                    </tr>
                      ...
                      {filteredProducts.map((p: any, idx) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {/* № */}
                            <td className="px-3 py-2.5 text-center text-xs font-bold text-muted-foreground">
                              {idx + 1}
                            </td>
                            {/* Kod */}
                            ...
                            {/* O'lchov */}
                            <td className="px-3 py-2.5 text-xs font-bold text-muted-foreground">
                              {unitLabel}
                            </td>
                            {/* Guruh */}
                            <td className="px-3 py-2.5">
                  ...
                  <div className="flex flex-col items-center justify-center py-28 text-muted-foreground/30">
                    ...
                    <p className="font-black uppercase tracking-widest text-xs">
                      {searchQuery
                        ...
                        : "Bu guruhda mahsulotlar yo'q"}
                    </p>
                    {canAdd && !searchQuery && (
                      <Button
                        size="sm"
                        className="mt-4 h-8 px-4 text-xs font-black rounded-lg gap-1.5 bg-primary text-white border-none opacity-60 hover:opacity-100"
                        onClick={handleAddNewClick}
                      >
                        <Plus className="w-3.5 h-3.5" /> Mahsulot qo'shish
                      </Button>
          ...
          <DialogContent className="rounded-[2rem] border-white/5 bg-card/40 backdrop-blur-3xl text-foreground max-w-lg p-0 shadow-2xl overflow-hidden">
            ...
            <div className="px-8 py-6 space-y-4">
              ...
              <div className="flex items-center gap-4">
                <Label className="w-36 text-xs text-right text-muted-foreground shrink-0">
                  Mahsulot nomi *:
                </Label>
                <Input
                  ...
                  onKeyDown={(e) => e.key === "Enter" && !isSaving && formData.name && handleSave()}
                />
              </div>
              {/* SKU */}
              <div className="flex items-center gap-4">
                ...
                <Input
                  className="h-9 flex-1 text-sm bg-muted/20 border-border/30 rounded-lg font-mono font-bold text-muted-foreground"
                  value={formData.sku}
                  disabled
                />
              </div>
              ...
              <div className="flex items-center gap-4">
                ...
                >
                  ...
                  <SelectContent className="rounded-xl">
                    ...
                    {categories?.map((cat) => (
                      <SelectItem key={cat.id} value={cat.id} className="text-sm font-bold">
                        {cat.name}
                      </SelectItem>
                    ))}
                  </SelectContent>
          ...
          <DialogContent className="rounded-[2rem] border-white/5 bg-card/40 backdrop-blur-3xl text-foreground max-w-sm p-8 shadow-2xl">
            <DialogHeader>
              ...
              <DialogTitle className="text-lg font-black">
                {editingCategory ? "Guruhni tahrirlash" : "Yangi guruh yaratish"}
              </DialogTitle>
            </DialogHeader>
            <div className="py-4 flex items-center gap-3">
              <Label className="text-xs text-muted-foreground w-24 text-right shrink-0">
                Guruh nomi:
              </Label>
              <Input
                className="h-9 flex-1 text-sm bg-background/80 border-border/40 rounded-lg font-bold"
                ...
                onKeyDown={(e) =>
                  e.key === "Enter" && !isSaving && newCategoryName.trim() && handleCreateCategory()
                }
              />
            </div>
            <DialogFooter className="gap-2">
              ...
              <Button
                className="h-10 px-5 rounded-xl bg-primary text-white font-black text-xs gap-2 border-none"
        ...
        >
          <DialogContent className="rounded-[2rem] border-white/5 bg-card/40 backdrop-blur-3xl text-foreground max-w-sm p-8 shadow-2xl text-center">
            ...
            <div className="flex gap-3">
              ...
              <Button
                className="flex-1 h-10 rounded-xl bg-rose-600 text-white font-black border-none"
