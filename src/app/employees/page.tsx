...
export default function StaffManagementPage() {
  ...
  // Qidiruv filtri (Xavfsiz qidiruv)
  ...
  // Saqlash va Yangilash funksiyasi
  const handleSubmit = async () => {
    if (!formData.email || !formData.name || !formData.surname) {
      toast({ title: "Xatolik", description: "Email, Ism va Familiya majburiy!", variant: "destructive" });
      return;
    }
            ...
            <DialogContent className="max-w-3xl p-0 overflow-hidden border-none shadow-2xl">
              <DialogHeader className="bg-slate-100 p-4 border-b">
                <DialogTitle className="text-sm font-bold text-slate-600 uppercase flex items-center gap-2">
                  <Lock className="w-4 h-4" /> {editingId ? "Tahrirlash: Personal kartochka" : "Yangi xodim qo'shish"}
                </DialogTitle>
              </DialogHeader>
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center gap-2 pt-2">
                        ...
                        <label htmlFor="isEmp" className="text-[10px] font-bold text-slate-600 uppercase cursor-pointer">Shtatda faol</label>
                      </div>
                    </div>
                  </div>
                </TabsContent>
              ...
              {filteredStaff.length > 0 ? (
                filteredStaff.map((worker) => (
                  <tr key={worker.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className="px-2 py-1 rounded text-[9px] font-bold bg-slate-100 text-slate-600">
                        {worker.role}
                      </span>
                    </td>
                ...
                <tr><td colSpan={5} className="p-10 text-center text-slate-400">Ma'lumot topilmadi</td></tr>
              )}
            </tbody>
          </table>
        </div>
