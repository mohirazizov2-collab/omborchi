...
export default function StockInPage() {
  ...
  // ✅ FIX: Date is now connected to state and used in movement data
  ...
  const handleProcess = async () => {
    if (!dnNumber || !supplier || !warehouseId) {
      toast({
        variant: "destructive",
        title: "Xatolik",
        description: "Barcha asosiy maydonlarni to'ldiring.",
      });
      return;
    }
    if (items.some((i) => !i.productId)) {
      toast({
        variant: "destructive",
        title: "Xatolik",
        description: "Ro'yxatdagi barcha mahsulotlarni tanlang.",
      });
      return;
    }
    ...
    try {
      ...
      // ✅ FIX: Use selected date from state, fall back to now
      const movementDate = movementDateStr
      ...
      for (const item of items) {
        ...
        if (invSnap.exists()) {
          ...
          // ✅ Already correct: create new inventory entry if doesn't exist
          await setDoc(invRef, {
            id: invId,
            warehouseId: warehouseId,
            productId: item.productId,
  ...
  const handleDownloadPDF = async () => {
    if (!processedInvoice) {
      toast({
        variant: "destructive",
        title: "Xatolik",
        description: "Hujjat ma'lumotlari topilmadi.",
      });
      return;
    }
        ...
        <div className="space-y-6">
          <Card className="border-none shadow-sm rounded-3xl bg-card/40 backdrop-blur-xl">
            <CardContent className="p-6 grid grid-cols-1 md:grid-cols-4 gap-6">
              ...
              {/* ✅ FIX: Date is now connected to state */}
              <div className="space-y-2">
                <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">
                  {t.common.date}
                </Label>
          ...
          <Card className="border-none shadow-sm rounded-3xl bg-card/40 backdrop-blur-xl overflow-hidden">
            <div className="p-6 border-b border-border/10 flex justify-between items-center bg-muted/10">
              ...
              <Button
                onClick={addItem}
                size="sm"
                className="rounded-xl h-9 px-4 font-black uppercase text-[10px] tracking-widest bg-primary text-white border-none"
              >
                    ...
                    <AnimatePresence mode="popLayout">
                      {items.map((item, index) => {
                        ...
                        const unitLabel = selectedProduct
                          ? t.units[
                              selectedProduct.unit as keyof typeof t.units
              ...
              <div className="flex gap-10">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-2xl font-black text-primary font-headline">
                    ...
                    <span className="text-xs">{t.settings.currency.split(" ")[0]}</span>
                  </span>
                </div>
              </div>
              <Button
          ...
          <DialogContent className="rounded-[2.5rem] border-white/5 bg-card/40 backdrop-blur-3xl text-foreground max-md p-8 shadow-2xl text-center">
            ...
            <DialogHeader>
              ...
              <p className="text-muted-foreground font-medium">
                Kirim nakladnoyi muvaffaqiyatli rasmiylashtirildi.
              </p>
            </DialogHeader>
