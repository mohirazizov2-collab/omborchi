...
const defaultForm: SaleForm = {
  customerName: "", staffId: "", paymentMethod: "Naqd", discount: 0, note: "",
};
 
const defaultFilter: FilterState = {
  payment: "all", staffId: "all", dateFrom: "", dateTo: "", search: "", status: "all",
};
...
export default function SalesPage() {
  ...
  [inventoryList, productSearch, categoryFilter]);
  ...
  const addToCart = useCallback((product: any) => {
    setCart((prev) => {
      const realStock = (product.quantity as number) ?? 0;
      const inCart    = prev.find((i) => i.productId === product.id)?.quantity ?? 0;
      if (realStock - inCart <= 0) {
        toast({ title: "Ombor yetarli emas", description: `"${product.name}" tugagan`, variant: "destructive" });
        return prev;
      }
      ...
      return [...prev, { productId: product.id, productName: product.name, productSku: product.sku || "", unitPrice, quantity: 1, subtotal: unitPrice }];
    });
  }, [toast]);
  ...
  const changeQty = useCallback((productId: string, delta: number) => {
    setCart((prev) =>
      prev.map((i) => {
        ...
        const newQty = Math.max(0, Math.min(i.quantity + delta, getStock(productId)));
  ...
  const handleSell = async () => {
    if (!db) return;
    if (!cart.length) { toast({ title: "Xatolik", description: "Savatga mahsulot qo'shing", variant: "destructive" }); return; }
    if (!form.staffId) { toast({ title: "Xatolik", description: "Sotuvchini tanlang", variant: "destructive" }); return; }
    if ((form.discount || 0) > cartSubtotal) { toast({ title: "Xatolik", description: "Chegirma summadan katta", variant: "destructive" }); return; }
    ...
    try {
      ...
      await runTransaction(db, async (tx) => {
        ...
        for (let i = 0; i < snaps.length; i++) {
          ...
          const qty: number = snaps[i].data().quantity ?? 0;
          if (qty < cart[i].quantity)
            throw new Error(`"${cart[i].productName}": so'ralgan ${cart[i].quantity} ta, mavjud ${qty} ta`);
        }
        ...
        for (let i = 0; i < cart.length; i++) {
          ...
          tx.set(txRefs[i], {
            ...
            type: "sale", quantityDelta: -item.quantity,
            quantityBefore: snaps[i].data().quantity, quantityAfter: snaps[i].data().quantity - item.quantity,
            unitPrice: item.unitPrice, subtotal: item.subtotal,
            staffId: form.staffId, staffName, saleId: saleRef.id, paymentMethod: form.paymentMethod,
            createdAt: serverTimestamp(),
          });
        }
      ...
      toast({ title: "✅ Sotuv amalga oshdi!", description: `${cart.length} xil — ${fmt(cartTotal)} so'm` });
      clearCart();
    } catch (err: any) {
  ...
  const handleDeleteSale = async (saleId: string) => {
    ...
    const returnItems: SaleItem[] = localSale.items?.length
      ? localSale.items
      : localSale.productId
        ? [{ productId: localSale.productId, productName: localSale.productName || "", productSku: localSale.productSku || "",
             unitPrice: localSale.unitPrice || 0, quantity: localSale.quantity || 1, subtotal: localSale.amount || 0 }]
        : [];
    ...
    try {
      ...
      await runTransaction(db, async (tx) => {
        ...
        for (let i = 0; i < returnItems.length; i++) {
          ...
          const curQty: number = invSnaps[i].data().quantity ?? 0;
          tx.update(productRefs[i], { quantity: increment(item.quantity), totalSold: increment(-item.quantity) });
          tx.set(refundTxRefs[i], {
            productId: item.productId, productName: item.productName, type: "sale_refund",
            quantityDelta: item.quantity, quantityBefore: curQty, quantityAfter: curQty + item.quantity,
            unitPrice: item.unitPrice, subtotal: item.subtotal,
            staffId: localSale.staffId, staffName: localSale.staffName,
            originalSaleId: saleId, createdAt: serverTimestamp(),
          });
        }
        tx.set(revenueRef, {
          revenue: increment(-(localSale.amount || 0)), salesCount: increment(-1),
  ...
  const hasFilters = Object.entries(filter).some(([k, v]) => ["payment","staffId","status"].includes(k) ? v !== "all" : v !== "");
  const setF = (key: keyof FilterState, val: string) => { setFilter((p) => ({ ...p, [key]: val })); setCurrentPage(1); };
 
  const exportCSV = () => {
    if (!filteredSales.length) return;
    const headers = ["Sana","Vaqt","Mijoz","Mahsulotlar","Soni","Chegirma","Jami","Sotuvchi","To'lov","Holat"];
    const rows = filteredSales.map((s: any) => [
      fmtDate(s.createdAt), fmtTime(s.createdAt), s.customerName||"",
      (s.items||[]).map((i: any) => `${i.productName}(${i.quantity})`).join("; ") || s.productName||"",
      s.totalItems||s.quantity||1, s.discount||0, s.amount||0, s.staffName||"", s.paymentMethod||"", s.status||"completed",
    ]);
          ...
          <div className="flex flex-1 overflow-hidden">
                ...
                {filteredProducts.length === 0 ? (
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-3">
                    {filteredProducts.map((product: any) => {
                      ...
                      const lowStock  = (product.quantity ?? 0) <= 5;
                      return (
                        <button
                          key={product.id}
                        ...
                        >
                          ...
                          <div className="p-2.5">
                            ...
                            <div className={`text-[10px] mt-0.5 font-semibold ${lowStock ? "text-red-500" : "text-slate-400"}`}>
                              {lowStock ? `⚠ ${product.quantity} ta` : `${product.quantity} ta`}
                            </div>
                          </div>
                        </button>
                      );
            ...
            <div className="w-[320px] xl:w-[360px] shrink-0 bg-white border-l border-slate-200 flex flex-col shadow-xl">
                    ...
                    {cart.map((item) => (
                      <div key={item.productId} className="px-4 py-3 flex items-start gap-2 hover:bg-slate-50/50 transition-colors">
                        ...
                        <div className="flex items-center gap-1 shrink-0 mt-0.5">
                          ...
                          <button
                            onClick={() => changeQty(item.productId, +1)}
                            disabled={item.quantity >= getStock(item.productId)}
              ...
              <div className="border-t border-slate-100 p-4 space-y-3 shrink-0 bg-slate-50">
                ...
                <button
                  onClick={handleSell}
                  disabled={loading || !cart.length || !form.staffId}
