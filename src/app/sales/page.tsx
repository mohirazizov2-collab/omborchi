    ...
    setCartItems((prev) => {
      ...
      if (existing) {
        ...
        if (existing.quantity >= maxQty) {
          toast({ title: "Ombor cheklangan", description: `Faqat ${maxQty} ta mavjud`, variant: "destructive" });
          return prev;
        }
        return prev.map((i) =>
  ...
  const handleAddSale = async () => {
    ...
    try {
      ...
      await runTransaction(db, async (transaction) => {
        // 1. Read all product docs & validate stock
        const productSnaps = await Promise.all(
          cartItems.map((item) => transaction.get(doc(db, "inventory", item.productId)))
        );
        productSnaps.forEach((snap, idx) => {
          if (!snap.exists()) throw new Error(`Mahsulot topilmadi: ${cartItems[idx].productName}`);
          const qty = snap.data().quantity || 0;
          if (qty < cartItems[idx].quantity) {
            throw new Error(`"${cartItems[idx].productName}" uchun faqat ${qty} ta mavjud`);
          }
        });
        ...
        transaction.set(saleRef, {
          ...
          // legacy compat fields (single item)
          productId:    cartItems[0].productId,
          productName:  cartItems[0].productName,
          productSku:   cartItems[0].productSku,
          unitPrice:    cartItems[0].unitPrice,
          quantity:     cartItems[0].quantity,
        });
 
        // 3. Update each product inventory + log transactions
        ...
        // 4. Log to daily_revenue for analytics
      ...
      toast({
        ...
        description: `${cartItems.length} xil mahsulot — jami ${fmt(cartTotal)} so'm`,
      });
  ...
  const handleDeleteSale = async (saleId: string) => {
    ...
    try {
      ...
      const items: SaleItem[] = sale.items || [
        { productId: sale.productId, quantity: sale.quantity || 1 } as SaleItem,
      ];
      items.forEach((item) => {
      ...
      const revenueDate = saleDate || today;
      batch.set(doc(db, "daily_revenue", revenueDate), {
        revenue:    increment(-(sale.amount || 0)),
        salesCount: increment(-1),
        itemsSold:  increment(-(sale.totalItems || sale.quantity || 1)),
      }, { merge: true });
 
      await batch.commit();
      toast({ title: "O'chirildi", description: `Inventar va daromad qaytarildi` });
    } catch (error: any) {
      toast({ title: "Xatolik", description: error?.message || "O'chirishda muammo", variant: "destructive" });
    }
  };
