...
export default function RecipesPage() {
  ...
  const [formData, setFormData] = useState({
    name: "",
    components: [{ id: generateId(), productId: "", quantity: 1 }]
  });
  ...
  const addComponent = () => {
    setFormData(prev => ({
      ...prev,
      components: [...prev.components, { id: generateId(), productId: "", quantity: 1 }]
    }));
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, "recipes", recipeId), {
        id: recipeId,
        name: formData.name,
        components: formData.components.map(c => ({ productId: c.productId, quantity: c.quantity })),
        createdAt: new Date().toISOString()
      });

      toast({ title: t.recipes.saveSuccess });
      setIsDialogOpen(false);
      setFormData({ name: "", components: [{ id: generateId(), productId: "", quantity: 1 }] });
    } catch (e) {
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            <DialogTrigger asChild>
              <Button className="gap-2 font-black uppercase tracking-widest text-[10px] h-12 px-8 rounded-2xl bg-primary text-white shadow-xl shadow-primary/20 border-none">
                <Plus className="w-4 h-4" /> {t.recipes.addNew}
              </Button>
            </DialogTrigger>
            <DialogContent className="rounded-[2.5rem] border-white/5 bg-card/40 backdrop-blur-3xl text-foreground max-w-2xl p-8 shadow-2xl">
              <DialogHeader>
                <DialogTitle className="text-2xl font-black flex items-center gap-3">
                  <FlaskConical className="text-primary w-6 h-6" /> {t.recipes.addNew}
                </DialogTitle>
              </DialogHeader>
              <div className="space-y-6 py-6">
                <div className="space-y-2">
                  <Label className="text-[10px] font-black uppercase tracking-widest opacity-50">{t.recipes.recipeName}</Label>
                  <Input 
                    className="h-12 rounded-2xl bg-background/50 font-bold"
                    placeholder="Masalan: 1 kv.m Kafel yotqizish"
                    value={formData.name}
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <Label className="text-[10px] font-black uppercase tracking-widest opacity-50">{t.recipes.components}</Label>
                    ...
                    {formData.components.map((comp) => (
                      <div key={comp.id} className="flex gap-3 items-center">
                        <div className="flex-1">
                          <Select 
                            onValueChange={(val) => updateComponent(comp.id, "productId", val)}
                            value={comp.productId}
                          >
                            <SelectTrigger className="h-11 rounded-xl bg-background/50 font-bold">
                        ...
                        <div className="w-24">
                          <Input 
                            type="number" 
                            className="h-11 rounded-xl bg-background/50 text-center font-bold"
                            value={comp.quantity}
                            onChange={(e) => updateComponent(comp.id, "quantity", parseFloat(e.target.value) || 0)}
                          />
                        </div>
                        <Button variant="ghost" size="icon" className="text-rose-500" onClick={() => removeComponent(comp.id)}>
          ...
          <CardContent className="p-4">
            <div className="relative">
              ...
              <Input 
                ...
                className="pl-12 h-12 rounded-2xl bg-background/50 border-border/40 font-bold" 
                value={searchQuery}
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredRecipes.map((recipe: any) => (
              <Card key={recipe.id} className="border-none glass-card bg-card/40 backdrop-blur-3xl rounded-[2.5rem] group overflow-hidden">
                <CardHeader className="pb-4">
                  <div className="flex justify-between items-start">
                    <CardTitle className="text-xl font-black tracking-tight">{recipe.name}</CardTitle>
                    <Button variant="ghost" size="icon" className="h-8 w-8 text-rose-500 opacity-0 group-hover:opacity-100" onClick={() => handleDelete(recipe.id)}>
                      <Trash2 className="w-4 h-4" />
                    </Button>
                  </div>
                </CardHeader>
                <CardContent>
                  <div className="space-y-3">
                    <p className="text-[10px] font-black uppercase text-muted-foreground opacity-50 tracking-widest mb-2">Tarkibi:</p>
                    {recipe.components.map((c: any, i: number) => {
                      const p = products?.find(prod => prod.id === c.productId);
                      return (
                        <div key={i} className="flex justify-between items-center text-xs p-3 rounded-xl bg-muted/20 border border-white/5">
                          <span className="font-bold truncate max-w-[150px]">{p?.name || 'Mahsulot'}</span>
                          <span className="font-black text-primary">{c.quantity} {p?.unit || ''}</span>
                        </div>
                      );
                    })}
                  </div>
